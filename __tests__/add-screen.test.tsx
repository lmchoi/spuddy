...
// ─── Save flow — subtitle in saved state ──────────────────────────────────────

describe('saved state subtitle', () => {
  it('shows the saved date in the header subtitle after saving', async () => {
    render(<AddScreen />);
    fireEvent.changeText(screen.getByDisplayValue(''), VALID_PASTE);
    await waitFor(() => expect(screen.getByText('Save 2 exercises')).toBeTruthy());
    await act(async () => { fireEvent.press(screen.getByText('Save 2 exercises')); });
...
describe('error banner', () => {
  ...
  it('shows error chip in header when parse fails entirely', async () => {
    render(<AddScreen />);
    fireEvent.changeText(screen.getByDisplayValue(''), `2026-05-22\nexercises: {\n  badline\n  alsoBad\n}`);
    await waitFor(() => expect(screen.getByText('2 err')).toBeTruthy());
  });
});

...
- **Sentry: crash context + notification debugging** — navigation tracking (route history on every crash), breadcrumbs on 5 key actions, and `captureMessage` when a notification is scheduled (passive audit trail for "notification didn't fire" reports). Plan: `docs/plans/sentry-instrumentation.md`.

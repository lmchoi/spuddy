...
5. ✅ **Debug helper** — "⚙ Fill sample" button on notes-import screen to seed test data without manual paste
6. ✅ **E2E flow** — `e2e/select-day.yaml` Maestro flow covering full happy path (import notes → select day → start session)
7. ✅ **Dead code removal** — `app/(tabs)/add.tsx` was hollowed out to a redirect; removed `styles/tabs/add.styles.ts` and its tests; E2E coverage confirms redirect via `accessibilityLabel="Add workout"`

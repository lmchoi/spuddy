  ...
  return db.transaction((tx) => {
    ...
    for (let ei = 0; ei < day.exercises.length; ei++) {
      ...
      tdb
        ...
        .values({ programDayId: dayRowId, exerciseIndex: ei, exerciseId, targetsJson: JSON.stringify(exercise.targets) })
        .run();
    }
  });
}
...
export async function addProgramDay(db: DrizzleDB, programId: number, day: ProgramDay): Promise<void> {
  return db.transaction((tx) => {
    const tdb = tx as unknown as DrizzleDB;
    ...
    for (let ei = 0; ei < day.exercises.length; ei++) {
      ...
      tdb
        .insert(programExercises)
        .values({ programDayId: dayRow.insertedId, exerciseIndex: ei, exerciseId, targetsJson: JSON.stringify(exercise.targets) })
        .run();
    }
  });
}
